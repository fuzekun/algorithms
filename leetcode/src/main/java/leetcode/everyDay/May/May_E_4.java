...
*   - 取对数 + double的处理
*   x = mul / k;
*   ans += lower_bound(0, i - 1, k);    // 以k结尾的
*
*   - 双指针
*
* 取对数，相乘就变成了对数的相加.
* log2 * 3 = log2 + log3
* 由于math.log会把mul变小。所以
*
* 复盘：
*  1. 先看会不会，能不能做。
*  2. 考虑整体流程
...
//            System.out.println("i = " + i + " j = " + j);
    ...
    public int numSubarrayProductLessThanK2(int[] nums, int k) {
        ...
        double b = Math.log(k);
        for (int i = 0; i < n; i++) {
            mul[i + 1] = mul[i] + Math.log(nums[i]);
//            System.out.println(mul[i + 1]);
            int l = 0, r = i + 1;
            while(l < r) {
                int mid = (l + r) >> 1;
                if (mul[i + 1] - mul[mid] + eps >= b) l = mid + 1;
                else r = mid;
            }
